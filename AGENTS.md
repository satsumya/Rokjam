...
1. Read **Standards.md** — it is the source of truth for forms, dates, locations, tags, bottom sheets, flow map upkeep, and more. Pay special attention to **content max width** (`layout.contentMaxWidth`) and **bottom nav** (`BottomNav` / `SessionBottomNav`) — product UI must not stretch full-bleed on wide viewports.
